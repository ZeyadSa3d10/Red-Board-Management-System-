...
import { BsExclamationTriangle, BsTrash, BsPencil, BsArrowDown, BsArrowUp } from 'react-icons/bs';
import api from '../../api/realApi';
import { useNotifications } from '../../context/NotificationContext';
import FilterBar from '../../components/common/FilterBar';
import FilterSearch from '../../components/common/FilterSearch';

const StockTable = ({ products, branchId, branches = [], onRefresh, onEdit, otherBranches = [], allStockMap = {}, userRole }) => {
  ...
  const filtered = useMemo(() => {
    ...
    if (categoryFilter) {
      result = result.filter(p => (p.categoryName || p.categoryId) === categoryFilter);
    }
    return result;
  ...
  const lowStockItems = useMemo(() =>
    products.filter(p => {
      const stock = branchId ? (p.branchStocks?.find(b => b.branchId === branchId)?.quantity || 0) : (p.totalQty || 0);
  ...
  return (
    <div>
      {lowStockItems.length > 0 && (
        <div className="card" style={{ padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8, borderColor: 'var(--color-warning)', background: 'var(--color-warning-light)' }}>
          <BsExclamationTriangle color="var(--color-warning)" size={20} />
          <span style={{ fontSize: '0.9rem' }}>{lowStockItems.length} منتجات تحت حد التنبيه</span>
        </div>
      )}

      <FilterBar variant="simple">
        <FilterSearch value={search} onChange={setSearch} placeholder="بحث بالاسم أو الباركود..." />
        <select className="form-control-custom" style={{ maxWidth: 200 }} value={categoryFilter}
        ...
        <table className="table-custom">
          <thead>
            <tr>
              ...
              <th className="th-sale">بالبيع</th>
              <th>الحالة</th>
          ...
          <tbody>
            {visibleItems.map(p => {
              ...
              const avgCost = branchId ? getBranchAvgCost(p, branchId) : (
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td className="mono td-sale">{formatCurrency(total * p.currentSalePrice)}</td>
                  <td>
