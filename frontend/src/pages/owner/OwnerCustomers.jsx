...
const OwnerCustomers = () => {
  ...
  useEffect(() => {
    api.getInvoices({ type: 'sale_deferred' }).then(inv => setDeferredInvoices(inv || [])).catch(() => {});
  }, []);
  ...
  const handleAddClient = async () => {
    ...
    try {
      ...
      load();
    } catch { addNotification('فشل إضافة العميل', 'danger'); }
  };
  ...
  const columns = [
    ...
    { key: 'usage', header: 'الاستخدام', sortable: false, render: (_, row) => {
      const usagePct = row.creditLimit > 0 ? Math.round((row.totalDeferred / row.creditLimit) * 100) : 0;
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        {row.totalDeferred > 0 && (
          <button className="btn-custom btn-custom-accent btn-custom-sm" onClick={(e) => {
            ...
            if (unpaid) { setSelectedDi(unpaid.deferredInvoiceId); setShowPayment(true); }
            else { addNotification('لا توجد فواتير آجلة مفتوحة لهذا العميل', 'info'); }
          }}>
  ...
  return (
    <div>
      <div className="page-header">
        <h2>العملاء</h2>
        <button className="btn-custom btn-custom-accent" onClick={() => setShowAddModal(true)}>إضافة عميل</button>
      </div>
      ...
      {selectedClient && (
        <div className="card" style={{ padding: 20, marginTop: 20 }}>
          ...
          <table className="table-custom">
            <thead>
              <tr>
                <th>رقم الفاتورة</th><th>الفرع</th><th>المبلغ الأصلي</th><th>المدفوع</th><th>المتبقي</th><th>تاريخ الاستحقاق</th><th>الحالة</th><th></th>
              </tr>
            </thead>
            <tbody>
